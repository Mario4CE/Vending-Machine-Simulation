...
"""
about: esta coloca la informacion de una ventana
Esta funcion crea una nueva ventana y contiene informacion del creador del proyecto
"""
def about():
    """
    Caracteristicas de la ventana del about(color de fondotamaño de la ventana, si se puede maximizar o no y
    su titulo)
    """
    ...
    """
    Informacion pedida, puesta e un label, se le da una letra distinta a la predeterminada y otras caracteristicas
    """
...
def resumen_tabla():
    return resumen_aux(0)
        
def resumen_aux(ind):
    if ind == len(movimientos):
        return []
    P = producto1(movimientos[ind][3],0)
    return [[movimientos[ind][3], P[1], movimientos[ind][4], P[6], movimientos[ind][5]]]  + resumen_aux(ind + 1)

################################################
################################################
    
"""
Se crea la vetana principal y se lan caracteristics de tamaño y el de poder o no minimizar o maximaizar pantalla
y se le pone un nombre, ademas se pone un label de fondo
"""
ventana = Tk()
...
leerD()
"""
Esta parte hace que funcione la ventanas y la seccion del menu, sin esta parte el codigo no correria(almenos
la parte grafica)
"""
